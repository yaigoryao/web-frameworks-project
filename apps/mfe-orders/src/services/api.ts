...
/** Локальная разработка: прямой доступ к сервисам. Значения подставляет webpack DefinePlugin. */
const AUTH_API_BASE_URL = process.env.AUTH_API_BASE_URL ?? 'http://localhost:3001';
const DATA_API_BASE_URL = process.env.DATA_API_BASE_URL ?? 'http://localhost:3002';
